...
export function exameOnUpdate(docSnapShot: any) {
    ...
    if ((docBeforeData.start as Timestamp).toDate().toLocaleString() != (docAfterData.start as Timestamp).toDate().toLocaleString()) {
        ...
        DatabaseReferences.updateDocumentWhereEquals('task', 'exameRef.id', docId, { 'end': docAfterData.start, 'started': null, 'lastSendAnswer': null, 'attempted': 0, 'isOpen': true });
    ...
    if ((docBeforeData.end as Timestamp).toDate().toLocaleString() != (docAfterData.end as Timestamp).toDate().toLocaleString()) {
        ...
        DatabaseReferences.updateDocumentWhereEquals('task', 'exameRef.id', docId, { 'end': docAfterData.end, 'started': null, 'lastSendAnswer': null, 'attempted': 0, 'isOpen': true });
